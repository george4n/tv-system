...
\nMonday to Friday from 09\:00 until 18\:00."
  if [[ $? = 1 ]]; then
    ...
    if [[ $szAnswer < 1 ]]; then
      ...
      echo "assistance needed by `getent passwd $USER | cut -d ':' -f 5 | cut -d ',' -f 1` ($USER) on $HOSTNAME. ***$szAnswer***" | /mnt/.system/kundservice/admin/server/slacktee.sh -p -c assistance -a danger
